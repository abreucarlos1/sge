...
function elementos()
{
	...
	this.remover = function (id_pai, id_filho)
	{
		...
		if(id_pai=="document.body")
		{
			...
			//document.body.removeChild(id_filho);
		}
		else
		{	
...
/**
 * Função que cria uma janela modal de acordo com os parametros
 * tipo = 1 - Informacao / 2 - Alerta,Erro / 3 - Personalizado
 * texto - Texto a ser apresentado
 * VERIFICAR O ARQUIVO html_conf.tpl
 * necessário Jquery e Bootstrap para funcionar
 */
 function mdAlert(tipo, texto, titulo, botoes)
 {
	var titulo = titulo != undefined ? titulo : '';
	var classe = '';
	var buttons = botoes != undefined ? botoes : '';
	var nModal = 0;
	var txtModal = '';
	var x = 0;
	...
	switch(tipo)
	{
		case 1:
			titulo = 'Informação';
			classe = 'color:blue;';
			//buttons = '<button type="button" class="btn btn-secondary" data-dismiss="modal">Fechar</button>';
			//buttons = '<button type="button" class="btn btn-secondary" onclick="mdClose('+nModal+');">Fechar</button>';
		break;
	...
	buttons += '<button type="button" class="btn btn-outline-secondary btn-sm" onclick="mdClose();">Fechar</button>';

	nModal = document.getElementById("nModal").value;

	nModal++;

	document.getElementById("nModal").value = nModal;

	txtModal ='<div class="modal fade" id="modalInfo_'+nModal+'" tabindex="-1" role="dialog" aria-labelledby="modalInfoTitle" aria-hidden="true">';
	txtModal +='	<div class="modal-dialog modal-dialog-centered" role="document">';
	txtModal +='		<div class="modal-content">';
	txtModal +='			<form name="frm_modal" id="frm_modal" method="POST">';
	txtModal +='				<div class="modal-header" id="modalInfoHeader_'+nModal+'">';
	txtModal +='				<h5 class="modal-title" id="modalInfoLongTitle_'+nModal+'">'+titulo+'</h5>';
	//txtModal +='				<button type="button" class="close" data-dismiss="modal" aria-label="Close">';
	txtModal +='				<button type="button" class="close" onclick="mdClose();" aria-label="Close">';
	txtModal +='					<span aria-hidden="true">&times;</span>';
	txtModal +='				</button>';
	txtModal +='				</div>';
	txtModal +='				<div class="modal-body" id="modalInfoBody_'+nModal+'" style="overflow: auto;">';
	
	txtModal += 				texto;				
	
	txtModal +='				</div>';
	txtModal +='				<div class="modal-footer" id="modalInfoFooter_'+nModal+'">';

	txtModal +=					buttons;

	//txtModal +='				<!-- <button type="button" class="btn btn-secondary" data-dismiss="modal">Fechar</button> -->';
	//txtModal +='				<!-- <button type="button" class="btn btn-primary">Save changes</button> -->';
	txtModal +='				</div>';
	txtModal +='			</form>';
	txtModal +='		</div>';
	txtModal +='	</div>';
	...
	document.getElementById("dvModal").innerHTML += txtModal;	

	//$('#modalInfoLongTitle_'+nModal).text(titulo);
	//$('#modalInfo').attr('id', 'modalInfo');
	$('#modalInfoLongTitle_'+nModal).attr('style',classe);
	$('#modalInfoHeader_'+nModal).attr('style','background: linear-gradient(rgba(252, 145, 6), rgb(248, 227, 200));');
	//$('#modalInfoBody_'+nModal).html(texto);
	//$('#modalInfoFooter_'+nModal).html(buttons);

	//$('#modalInfo_'+nModal).css('z-index', 1040 + (10 * nModal));

	//mostra o modal
	$('#modalInfo_'+nModal).modal('show');
 ...
 function initModal(nModal)
 {

	var x = 0;

	 $('#modalInfo_'+nModal).on('shown.bs.modal', function ( event ) {
			...
			$(this).addClass('fv-modal-stack');
			 ...
			 $(this).css('z-index', 1040 + (10 * nModal));
 
			 $('.modal-backdrop').not('.fv-modal-stack')
					 .css( 'z-index', 1039 + (10 * nModal));
 
			 $('.modal-backdrop').not('fv-modal-stack')
					 .addClass( 'fv-modal-stack' );
					 
			x = document.getElementsByClassName('modal-backdrop');

			//alert(x.length);

			x[(x.length)-1].id = 'backdrop_'+nModal;
 ...
 function mdClose()
 {
	...
	if(nModal>0)
	{
		...
		$('#modalInfo_'+nModal).modal('hide');
		
		$('#modalInfo_'+nModal).remove();

		$('#backdrop_'+nModal).remove();	
