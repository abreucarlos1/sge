...
class xajaxResponse
{
	...
	// ADICIONADO EM 04/04/2007 PARA TRATAR COMBOBOX
	/*
	
	I use addNewOptions if I want to clear the current select box first before adding any values.
	If adding multiple options, I can add the optional "sCurrentID", which sets sets the matching value as the currently selected option.
	
	IE, if I call
	addNewOptions('mymainselectfield', $options, '15');
	then if the option being added has a value of 15 it will be set as the currently selected.
		
	
	*/
	
	 function addAddOption($sSelectId, $sOptionText, $sOptionValue, $sSetCurrent = NULL)
	  {  
	
		$sScript  = "var objOption = new Option('".addslashes($sOptionText)."','".$sOptionValue."');";
		$sScript .= "document.getElementById('".$sSelectId."').options.add(objOption);";
		
		$this->addScript($sScript);		
		
		if (!is_null($sSetCurrent))
		{
			$number = $sSetCurrent - 1;
			
			//$this->addAlert($sSelectId.'---'.$number);

			$sScript = "document.getElementById('".$sSelectId."').selectedIndex = ".$number.";\n";
	  ...
	  function addCreateOptions($sSelectId, $aOptions, $sCurrentID = NULL , $first_element=true)
		{
			if($first_element)
			{
				$this->addAddOption($sSelectId, 'SELECIONE', '0');
			}
			...
			foreach($aOptions as $sOptionText => $sOptionValue)
			{
				...
				//$this->addAlert($sOptionValue.'-'.$sCurrentID);
				
				if ($sOptionValue == $sCurrentID)
				{
					$sSetCurrent = $currentindex+1;					
				}
				
				//$this->addAlert($sSetCurrent);
				
				$this->addAddOption($sSelectId, $sOptionText, $sOptionValue, $sSetCurrent);
	  ...
	  function addNewOptions($sSelectId, $aOptions, $sCurrentID = NULL, $first_element=true)
		{
			$sScript = "document.getElementById('".$sSelectId."').length = 0;";
			//$sScript = "for(i = document.getElementById('".$sSelectId."').length;i>0;i--){document.getElementById('".$sSelectId."').options[i] = null;}";
						
			$this->addScript($sScript);
	
			$this->addCreateOptions($sSelectId, $aOptions, $sCurrentID, $first_element);
	
		}
	
		// FIM ADICIONADO EM 04/04/2007 PARA TRATAR COMBOBOX
